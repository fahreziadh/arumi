...
export const PROMPT_DEFS = {
	...
	coachSubmission: {
		...
		template: [
			...
			"- REGISTER: the conversation above sets the target, not some ideal. Writing at roughly the formality {{personaName}} is using is right, and casual contractions, dashes, exclamation marks, and short fragments are normal in chat apps rather than tone problems there.",
			"- RAISE a tone tip when the wording would land on {{personaName}} differently from how the learner meant it: blunter or more demanding than intended, colder or more distant than the relationship, noticeably stiffer or more formal than the room, over-familiar with someone senior or barely known, or apologetic and self-diminishing where a plain statement belongs. Flag these even when the grammar is perfect. This is the one thing the learner cannot hear on their own, so it is the most valuable tip you can give.",
			"- Do NOT raise a tone tip merely to offer a snappier, more idiomatic or more confident version of a sentence that already lands as intended. That belongs in the rewrite, which the learner opens only if they want it.",
			"- THE TEST for a tone tip: picture {{personaName}} reading this message. Would they feel something the learner did not intend, a little pushed, held at arm's length, or talked up to? Then raise it. Would they simply read a fine message you might have phrased differently? Then say nothing.",
			"- Between friends on a chat app the tolerance is wide. Direct requests are normal there: \"give me some ideas\", \"send me the link\", \"tell me more\" are ordinary English, and adding \"please\" makes them polite. Never mark a polite request as sounding like a command.",
			"- Hedges such as \"I think\", \"maybe\", \"I was wondering\" are normal, polite English. Mark them only when they stack up enough to make a simple request sound unsure of itself.",
			...
			"- quote MUST be copied verbatim from the message, and as short as it can be while still showing the problem: usually a word or a phrase. When the problem is how the message as a whole lands (an abrupt opening, a missing greeting, a demand with no framing), quote the single sentence that carries it, never the whole message.",
	...
	coachDraft: {
		...
		template: [
			...
			"- REGISTER: the conversation above sets the target. Writing at roughly the formality {{personaName}} uses is right, and casual contractions, dashes and fragments are normal in chat apps.",
			"- RAISE a tone tip when the wording would land on {{personaName}} differently from how the learner meant it: blunter or more demanding than intended, colder than the relationship, stiffer than the room, over-familiar with someone senior, or apologetic where a plain statement belongs. Flag these even when the grammar is perfect.",
			"- Do NOT mark a sentence that already lands as intended just to offer a snappier version, and do not mark hedges like \"I think\" or \"maybe\" as weak unless they stack up enough to make a request sound unsure of itself.",
			"- THE TEST: picture {{personaName}} reading this. Would they feel something the learner did not intend, a little pushed or held at arm's length? Then raise it. Would they just read a fine message you might have phrased differently? Then say nothing.",
			"- Accept British and American spelling equally; neither is ever an error.",
			"- quote MUST be copied verbatim from the draft, and as short as it can be while still showing the problem: usually a word or a phrase. When the problem is how the draft lands as a whole, quote the single sentence that carries it, never the whole draft.",
